...
function getListOrder(status) {
    ...
    let conditionStatus = status ? ("&status=" + status) : "";
    //   ------------------------------------- API DANH SÁCH ORDER -------------------------------------
    $.ajax({
        url: baseUrlOrder + "/get-by-status?accountId=" + userId + conditionStatus,
        type: "GET",
...
function fillOrderToTable(orderList, status) {
    ...
    for (var index = 0; index < orderList.length; index++) {
        ...
        let textStatus = (order.status === 'PENDING' && status === 'PENDING') ?
            `<div class="row">
                        <div class="col-6">
        ...
        $("#order-item").append(
            ...
            <div class="col-5 d-flex justify-conten-start align-items-center">
                <div class="">
                    <h4 class="p-2"><b>`+ order.product.name + `</b></h4>
