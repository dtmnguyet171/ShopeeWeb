...
function buildManager() {
  if ("CUSTOMER" === localStorage.getItem("role") || localStorage.getItem("role") === null) {
...
// Get the input field
var input = document.getElementById("product_name_search");
...
function fillProductsToTable(productList) {
  ...
  for (var index = 0; index < productList.length; index++) {
    ...
    let statusProduct = product.status === "NEW" ? 'Mới' : 'Đã sử dụng';
    ...
    <div href="#" class="home-product-item">
      ...
      <div class="home-product-item__price">
        <span class="home-product-item__price-current">Danh mục: `+ product.type + `</span>
      </div>
      ...
      ${isManagerProduct ?
        ...
        <div>
        <a data-target="#modalProduct" onclick="editProduct('${product.id}', '${product.image}', '${product.name}', 
        '${product.price}', '${product.shippingUnit}', '${product.status}', '${product.type}')">
...
function getProductName() {
  productName = document.getElementById("product_name_search").value;
}
